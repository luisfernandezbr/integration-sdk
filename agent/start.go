...
const (
	...
	// StartModelIntegrationInstanceIDColumn is the column json value integration_instance_id
	StartModelIntegrationInstanceIDColumn = "integration_instance_id"
...
// toStartTypePointer is the enumeration pointer type for type
func toStartTypePointer(v int32) *StartType {
	nv := StartType(v)
	return &nv
}

// toStartTypeEnum is the enumeration pointer wrapper for type
func toStartTypeEnum(v *StartType) string {
	if v == nil {
		return toStartTypePointer(0).String()
	}
	return v.String()
}
...
func (o *Start) ToMap() map[string]interface{} {
	...
	return map[string]interface{}{
		...
		"integration_instance_id": toStartObject(o.IntegrationInstanceID, true),
	...
	for k, v := range kv {
		if v == nil || reflect.ValueOf(v).IsZero() {
			...
			if k == "last_export_date" {
				if dt, ok := v.(*StartLastExportDate); ok {
					if dt.Epoch == 0 && dt.Offset == 0 && dt.Rfc3339 == "" {
						delete(kv, k)
					}
				}
...
// MarshalJSON returns the bytes for marshaling to json
func (o *StartPartial) MarshalJSON() ([]byte, error) {
	return json.Marshal(o.ToMap())
}

// UnmarshalJSON will unmarshal the json buffer into the object
func (o *StartPartial) UnmarshalJSON(data []byte) error {
	kv := make(map[string]interface{})
	if err := json.Unmarshal(data, &kv); err != nil {
		return err
	}
...
func (o *StartPartial) setDefaults(frommap bool) {
}

// FromMap attempts to load data into object from a map
func (o *StartPartial) FromMap(kv map[string]interface{}) {
	if val, ok := kv["architecture"].(*string); ok {
		o.Architecture = val
	...
	if o.LastExportDate == nil {
		o.LastExportDate = &StartLastExportDate{}
	}
	...
	if val, ok := kv["type"].(*StartType); ok {
		o.Type = val
	} else if val, ok := kv["type"].(StartType); ok {
		o.Type = &val
	} else {
		if val, ok := kv["type"]; ok {
			if val == nil {
				o.Type = toStartTypePointer(0)
			} else {
				// if coming in as map, convert it back
				if kv, ok := val.(map[string]interface{}); ok {
					val = kv["StartType"]
				...
				if em, ok := val.(string); ok {
					...
					case "enroll", "ENROLL":
						o.Type = toStartTypePointer(0)
					case "ping", "PING":
						o.Type = toStartTypePointer(1)
					case "crash", "CRASH":
						o.Type = toStartTypePointer(2)
					case "log", "LOG":
						o.Type = toStartTypePointer(3)
					case "integration", "INTEGRATION":
						o.Type = toStartTypePointer(4)
					case "export", "EXPORT":
						o.Type = toStartTypePointer(5)
					case "project", "PROJECT":
						o.Type = toStartTypePointer(6)
					case "repo", "REPO":
						o.Type = toStartTypePointer(7)
					case "user", "USER":
						o.Type = toStartTypePointer(8)
					case "calendar", "CALENDAR":
						o.Type = toStartTypePointer(9)
					case "uninstall", "UNINSTALL":
						o.Type = toStartTypePointer(10)
					case "upgrade", "UPGRADE":
						o.Type = toStartTypePointer(11)
					case "start", "START":
						o.Type = toStartTypePointer(12)
					case "stop", "STOP":
						o.Type = toStartTypePointer(13)
					case "pause", "PAUSE":
						o.Type = toStartTypePointer(14)
					case "resume", "RESUME":
						o.Type = toStartTypePointer(15)
					}
				}
			}
		}
