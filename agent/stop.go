...
// toStopTypePointer is the enumeration pointer type for type
func toStopTypePointer(v int32) *StopType {
	nv := StopType(v)
	return &nv
}

// toStopTypeEnum is the enumeration pointer wrapper for type
func toStopTypeEnum(v *StopType) string {
	if v == nil {
		return toStopTypePointer(0).String()
	}
	return v.String()
}
...
func (v *StopType) FromInterface(o interface{}) error {
	switch val := o.(type) {
	case StopType:
		*v = val
	case int32:
		*v = StopType(int32(val))
	case int:
		*v = StopType(int32(val))
	case string:
		switch val {
...
func (o *Stop) SetCustomerID(id string) {
	...
	for k, v := range kv {
		if v == nil || reflect.ValueOf(v).IsZero() {
			...
			if k == "last_export_date" {
				if dt, ok := v.(*StopLastExportDate); ok {
					if dt.Epoch == 0 && dt.Offset == 0 && dt.Rfc3339 == "" {
						delete(kv, k)
					}
				}
...
// MarshalJSON returns the bytes for marshaling to json
func (o *StopPartial) MarshalJSON() ([]byte, error) {
	return json.Marshal(o.ToMap())
}

// UnmarshalJSON will unmarshal the json buffer into the object
func (o *StopPartial) UnmarshalJSON(data []byte) error {
	kv := make(map[string]interface{})
	if err := json.Unmarshal(data, &kv); err != nil {
		return err
	}
...
func (o *StopPartial) setDefaults(frommap bool) {
}

// FromMap attempts to load data into object from a map
func (o *StopPartial) FromMap(kv map[string]interface{}) {
	if val, ok := kv["architecture"].(*string); ok {
		o.Architecture = val
	...
	if o.LastExportDate == nil {
		o.LastExportDate = &StopLastExportDate{}
	}
	...
	if val, ok := kv["type"].(*StopType); ok {
		o.Type = val
	} else if val, ok := kv["type"].(StopType); ok {
		o.Type = &val
	} else {
		if val, ok := kv["type"]; ok {
			if val == nil {
				o.Type = toStopTypePointer(0)
			} else {
				// if coming in as map, convert it back
				if kv, ok := val.(map[string]interface{}); ok {
					val = kv["StopType"]
				...
				if em, ok := val.(string); ok {
					...
					case "enroll", "ENROLL":
						o.Type = toStopTypePointer(0)
					case "ping", "PING":
						o.Type = toStopTypePointer(1)
					case "crash", "CRASH":
						o.Type = toStopTypePointer(2)
					case "log", "LOG":
						o.Type = toStopTypePointer(3)
					case "integration", "INTEGRATION":
						o.Type = toStopTypePointer(4)
					case "export", "EXPORT":
						o.Type = toStopTypePointer(5)
					case "project", "PROJECT":
						o.Type = toStopTypePointer(6)
					case "repo", "REPO":
						o.Type = toStopTypePointer(7)
					case "user", "USER":
						o.Type = toStopTypePointer(8)
					case "calendar", "CALENDAR":
						o.Type = toStopTypePointer(9)
					case "uninstall", "UNINSTALL":
						o.Type = toStopTypePointer(10)
					case "upgrade", "UPGRADE":
						o.Type = toStopTypePointer(11)
					case "start", "START":
						o.Type = toStopTypePointer(12)
					case "stop", "STOP":
						o.Type = toStopTypePointer(13)
					case "pause", "PAUSE":
						o.Type = toStopTypePointer(14)
					case "resume", "RESUME":
						o.Type = toStopTypePointer(15)
					}
				}
			}
		}
