...
// Package integration - the pipeline models
package integration
...
const (
	...
	UserTopic datamodel.TopicNameType = "pipeline_integration_User_topic"

	// UserStream is the default stream name
	UserStream datamodel.TopicNameType = "pipeline_integration_User_stream"

	// UserTable is the default table name
	UserTable datamodel.TopicNameType = "pipeline_integration_User"

	// UserModelName is the model name
	UserModelName datamodel.ModelNameType = "pipeline.integration.User"
)
...
func (o *User) String() string {
	return fmt.Sprintf("pipeline.integration.User<%s>", o.ID)
}
...
func GetUserAvroSchemaSpec() string {
	spec := map[string]interface{}{
		"type":      "record",
		"namespace": "pipeline.integration",
		"name":      "User",
		"fields": []map[string]interface{}{
			...
			map[string]interface{}{
				"name": "groups",
				"type": map[string]interface{}{"items": map[string]interface{}{"type": "record", "name": "groups", "fields": []interface{}{map[string]interface{}{"type": "string", "name": "group_id", "doc": "Group id"}, map[string]interface{}{"doc": "Group name", "type": "string", "name": "name"}}, "doc": "Group names"}, "type": "array", "name": "groups"},
			},
			map[string]interface{}{
				"name": "id",
				"type": "string",
...
func NewUserInputStreamDir(dir string, errors chan<- error, transforms ...TransformUserFunc) (chan User, <-chan bool) {
	files, err := fileutil.FindFiles(dir, regexp.MustCompile("/pipeline.integration/user\\.json(\\.gz)?$"))
	if err != nil {
		errors <- err
		ch := make(chan User)
		close(ch)
...
func NewUserOutputStreamDir(dir string, ch chan User, errors chan<- error, transforms ...TransformUserFunc) <-chan bool {
	fp := filepath.Join(dir, "/pipeline.integration/user\\.json(\\.gz)?$")
		...
		for {
			...
			case item := <-ch:
				...
				if object, ok := item.Object().(*User); ok {
					...
					errors <- fmt.Errorf("invalid event received. expected an object of type pipeline.integration.User but received on of type %v", reflect.TypeOf(item.Object()))
				}
			}
		}
	}()
		...
		OnDataReceived: func(msg eventing.Message) error {
			...
			case eventing.JSONEncoding:
				if err := json.Unmarshal(msg.Value, &object); err != nil {
					return fmt.Errorf("error unmarshaling json data into pipeline.integration.User: %s", err)
				}
			case eventing.AvroEncoding:
				if err := object.FromAvroBinary(msg.Value); err != nil {
					return fmt.Errorf("error unmarshaling avro data into pipeline.integration.User: %s", err)
				}
			default:
				return fmt.Errorf("unsure of the encoding since it was not set for pipeline.integration.User")
			}
