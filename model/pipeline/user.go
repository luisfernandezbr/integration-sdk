...
const (
	...
	UserTopic datamodel.TopicNameType = "pipeline_User_topic"

	// UserStream is the default stream name
	UserStream datamodel.TopicNameType = "pipeline_User_stream"

	// UserTable is the default table name
	UserTable datamodel.TopicNameType = "pipeline_User"

	// UserModelName is the model name
	UserModelName datamodel.ModelNameType = "pipeline.User"
)
...
func (o *User) String() string {
	return fmt.Sprintf("pipeline.User<%s>", o.ID)
}
...
func NewUserInputStreamDir(dir string, errors chan<- error, transforms ...TransformUserFunc) (chan User, <-chan bool) {
	files, err := fileutil.FindFiles(dir, regexp.MustCompile("/pipeline/user\\.json(\\.gz)?$"))
	if err != nil {
		errors <- err
		ch := make(chan User)
		close(ch)
...
func NewUserOutputStreamDir(dir string, ch chan User, errors chan<- error, transforms ...TransformUserFunc) <-chan bool {
	fp := filepath.Join(dir, "/pipeline/user\\.json(\\.gz)?$")
	...
	go func() {
		...
		for item := range ch {
			if object, ok := item.Object().(*User); ok {
				...
				errors <- fmt.Errorf("invalid event received. expected an object of type pipeline.User but received on of type %v", reflect.TypeOf(item.Object()))
			}
		}
	}()
	return done
	...
	consumer.Consume(&eventing.ConsumerCallbackAdapter{
		OnDataReceived: func(msg eventing.Message) error {
			...
			if err := json.Unmarshal(msg.Value, &object); err != nil {
				return fmt.Errorf("error unmarshaling json data into pipeline.User: %s", err)
			}
