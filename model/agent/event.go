...
func NewEventProducer(producer event.Producer, ch <-chan datamodel.ModelSendEvent, errors chan<- error) <-chan bool {
	done := make(chan bool, 1)
	go func() {
...
func NewEventConsumer(consumer event.Consumer, ch chan<- datamodel.ModelReceiveEvent, errors chan<- error) {
...
func (e *EventReceiveEvent) Message() event.Message {
	return e.message
}
...
func (o *Event) NewProducerChannel(producer event.Producer, errors chan<- error) datamodel.ModelEventProducer {
...
func NewEventProducerChannel(producer event.Producer, errors chan<- error) datamodel.ModelEventProducer {
	ch := make(chan datamodel.ModelSendEvent)
	return &EventProducer{
		ch:   ch,
		done: NewEventProducer(producer, ch, errors),
...
func (o *Event) NewConsumerChannel(consumer event.Consumer, errors chan<- error) datamodel.ModelEventConsumer {
...
func NewEventConsumerChannel(consumer event.Consumer, errors chan<- error) datamodel.ModelEventConsumer {
	ch := make(chan datamodel.ModelReceiveEvent)
	NewEventConsumer(consumer, ch, errors)
	return &EventConsumer{
		ch: ch,
