...
func (o *Sprint) String() string {
	return fmt.Sprintf("work.v1.Sprint<%s>", o.ID)
}
...
// CreateSprintKQLStreamSQL creates KQL Stream SQL for Sprint
func CreateSprintKQLStreamSQL() string {
	var builder strings.Builder
	builder.WriteString(fmt.Sprintf("CREATE STREAM %s ", SprintDefaultStream))
	builder.WriteString(fmt.Sprintf("WITH (KAFKA_TOPIC='%s', VALUE_FORMAT='AVRO', KEY='id'", SprintDefaultTopic))
	builder.WriteString(", TIMESTAMP='started_ts'")
	builder.WriteString(");")
	return builder.String()
}

// CreateSprintKQLTableSQL creates KQL Table SQL for Sprint
func CreateSprintKQLTableSQL() string {
	var builder strings.Builder
	builder.WriteString(fmt.Sprintf("CREATE TABLE %s ", SprintDefaultTable))
	builder.WriteString(fmt.Sprintf("WITH (KAFKA_TOPIC='%s', VALUE_FORMAT='AVRO', KEY='id'", SprintDefaultTopic))
	builder.WriteString(", TIMESTAMP='started_ts'")
	builder.WriteString(");")
	return builder.String()
...
func CreateSprintInputStreamDir(dir string, errors chan<- error, transforms ...TransformSprintFunc) (chan Sprint, <-chan bool) {
	files, err := fileutil.FindFiles(dir, regexp.MustCompile("/work/v1/sprint\\.json(\\.gz)?$"))
	if err != nil {
		errors <- err
		ch := make(chan Sprint)
		close(ch)
...
func CreateSprintOutputStreamDir(dir string, ch chan Sprint, errors chan<- error, transforms ...TransformSprintFunc) <-chan bool {
	fp := filepath.Join(dir, "/work/v1/sprint\\.json(\\.gz)?$")
...
func CreateSprintProducer(producer util.Producer, ch chan Sprint, errors chan<- error) <-chan bool {
	...
	go func() {
		...
		schemaspec := CreateSprintAvroSchemaSpec()
