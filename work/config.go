...
func FindConfigs(client graphql.Client, input *ConfigQueryInput) (*ConfigConnection, error) {
	...
	var sb strings.Builder
	sb.WriteString("query ConfigQueryMany($first: Int, $last: Int, $before: Cursor, $after: Cursor, $query: QueryInput) {\n")
	sb.WriteString("\twork {\n")
	sb.WriteString("\t\tConfigs(first: $first last: $last before: $before after: $after query: $query) {\n")
